...
class Dispatch {
	...
	static inline var CLASSNAME = 'om.web.Dispatch';
	static var PACK = ['om','web'];
	...
	public function new( url : String, params : Map<String,String> ) {
		parts = url.split( "/" );
	...
	public macro function getParams( ethis : Expr ) : Expr {
		...
		var rt = TPath( { pack : ["haxe", "macro"], name : "MacroType", params : [TPExpr(Context.parse( CLASSNAME+".getRunParam("+index+")",p))], sub : null } );
	...
	static inline var PREFIX = CLASSNAME+".DispatchRule.";
	...
	static function makeConfig( obj : Expr ) {
		...
		case TInst(i, _):
			...
			return { expr : EUntyped ({ expr : ECall({ expr : EField(macro om.web.Dispatch,"extractConfig"), pos : p },[obj]), pos : p }), pos : p };
		default:
